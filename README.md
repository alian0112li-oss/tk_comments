...
4. 导出三选一：
   - **📊 导出 Excel(树形)** —— 标准 `.xlsx`，行按树形深度优先排序（回复紧跟父评论），带「层级」「缩进正文」「回复给」「父评论ID」等列，上下文关系一眼可见。零依赖生成（内置 `src/xlsx.js`），Excel/WPS 直接打开。
   - **导出 JSON(树)** —— 带 `children` 的嵌套结构。
   - **导出 CSV** —— 扁平表（含 `parent_id`/`level` 关系列）。
