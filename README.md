...
面向"给一个品、拿到仿写成品"的场景，全程不用写内容：

1. 在 TikTok 搜索**产品关键词**，打开一个相关视频，展开评论区。
2. 扩展 →「▶ 开始采集」，等评论采完。
3. 点 **「📝 生成仿写提示词（复制）」** —— 工具把所有分组 + 固定的风格规则拼成一段提示词并复制到剪贴板（剪贴板不可用时自动下载为 `仿写提示词.txt`）。
4. 把提示词**粘贴到 Claude**，Claude 直接按相同的「组号/层级/缩进」格式输出仿写成品。
5. 复制 Claude 的输出即为交付物（**只含仿写结果**）。

> 风格规则已固化在提示词里：保持与原组相同的结构和回复关系、自然口语化日语、保留 emoji、安利柔和不强推、只输出结果。制作人员无需改任何字。

---

### 导出（原始数据，供核对/备份）

导出三选一（均已精简为「评论内容 + 回复关系」，并**按组**组织）：
   > 分组规则：一条顶层评论 + 它的所有回复 = 一组；没有回复的评论自成一组。
   - **📊 导出 Excel(按组)** —— 标准 `.xlsx`，列：`分组 | 层级 | 评论内容(缩进) | 昵称 | 回复给`；同组共用组号，组间空行分隔。零依赖生成（内置 `src/xlsx.js`），Excel/WPS 直接打开。
   - **导出 JSON(树)** —— 数组，每个元素 `{ group, comment }`，`comment` 节点含 `text / nickname / reply_to / replies`。
   - **导出 CSV** —— 同 Excel 的五列。
